...
module "lambda_api_gateway" {
  ...
  ENVIRONMENT_VARIABLES               = merge({ FIREHOSE_STREAM_NAME = module.centralized_logs.KINESIS_FIREHOSE_DELIVERY_STREAM_NAME, content_bucket = module.content_management_bucket.CONTENT_BUCKET, mem_host = module.elasticache_memcached.ELASTICACHE_NODE_ADDRESS[0], mem_port = module.elasticache_memcached.ELASTICACHE_NODE_PORT_NUMBER[0] }, var.ENVIRONMENT_VARIABLES_LAMBDA_API)
